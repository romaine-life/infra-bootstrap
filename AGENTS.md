...
- **AKS** (`infra-aks`) — Free tier, system-mode pool is `system` at 3x Standard_E2bs_v5 (2 vCPU, 16 GiB RAM each) with 128 GiB OS disks, Azure CNI Overlay, workload identity. A drained legacy default pool named `tmp` may briefly exist during migration and should be deleted once the named `system` pool is verified healthy.
